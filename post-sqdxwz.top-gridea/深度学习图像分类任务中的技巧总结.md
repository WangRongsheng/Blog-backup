计算机视觉主要问题有图像分类、目标检测和图像分割等。针对图像分类任务，提升准确率的方法路线有两条，一个是模型的修改，另一个是各种数据处理和训练的技巧(`tricks`)。图像分类中的各种技巧对于目标检测、图像分割等任务也有很好的作用，因此值得好好总结。本文在精读论文的基础上，总结了图像分类任务的各种`tricks`如下：
...
学习率是神经网络训练中最重要的超参数之一，针对学习率的技巧有很多。`Warm up`是在ResNet论文[1]中提到的一种学习率预热的方法。由于刚开始训练时模型的权重(weights)是随机初始化的(全部置为0是一个坑，原因见[2])，此时选择一个较大的学习率，可能会带来模型的不稳定。学习率预热就是在刚开始训练的时候先使用一个较小的学习率，训练一些epoches或iterations，等模型稳定时再修改为预先设置的学习率进行训练。论文[1]中使用一个110层的ResNet在cifar10上训练时，先用0.01的学习率训练直到训练误差低于80%(大概训练了400个iterations)，然后使用0.1的学习率进行训练。
...
[![BX6TNq.jpg](https://s1.ax1x.com/2020/11/11/BX6TNq.jpg)](https://imgchr.com/i/BX6TNq)

其中，ε是一个小的常数，K是类别的数目，y是图片的真正的标签，i代表第i个类别，$q_i$是图片为第i类的概率。
...
[![BXcQ8f.jpg](https://s1.ax1x.com/2020/11/11/BXcQ8f.jpg)](https://imgchr.com/i/BXcQ8f)

如下图所示，Ix, Iy是原始图片的宽和高。w和h称为boundary position，它决定了四个裁剪得到的小图片的尺寸。w和h从beta分布Beta(β, β)中随机生成，β也是RICAP的超参数。最终拼接的图片尺寸和原图片尺寸保持一致。

[![BXcwGV.jpg](https://s1.ax1x.com/2020/11/11/BXcwGV.jpg)](https://imgchr.com/i/BXcwGV)

[![BXctVs.jpg](https://s1.ax1x.com/2020/11/11/BXctVs.jpg)](https://imgchr.com/i/BXctVs)
...
[![BXcyqJ.jpg](https://s1.ax1x.com/2020/11/11/BXcyqJ.jpg)](https://imgchr.com/i/BXcyqJ)
...
[![BXgVzT.jpg](https://s1.ax1x.com/2020/11/11/BXgVzT.jpg)](https://imgchr.com/i/BXgVzT)
...
[![BXgaeH.png](https://s1.ax1x.com/2020/11/11/BXgaeH.png)](https://imgchr.com/i/BXgaeH)
...
[![BjWXa4.jpg](https://s1.ax1x.com/2020/11/11/BjWXa4.jpg)](https://imgchr.com/i/BjWXa4)
...
[![Bjfpxx.jpg](https://s1.ax1x.com/2020/11/11/Bjfpxx.jpg)](https://imgchr.com/i/Bjfpxx)
...
[![BjfdyV.jpg](https://s1.ax1x.com/2020/11/11/BjfdyV.jpg)](https://imgchr.com/i/BjfdyV)
...
[![BjfcWR.jpg](https://s1.ax1x.com/2020/11/11/BjfcWR.jpg)](https://imgchr.com/i/BjfcWR)

另外，这种方法相对于SGD对超参数的变化不是那么敏感，也就是说鲁棒性更好。但是，针对不同的问题还是需要调节超参数的，只是所用的时间可能变少了。

[![BjfWy6.jpg](https://s1.ax1x.com/2020/11/11/BjfWy6.jpg)](https://imgchr.com/i/BjfWy6)
